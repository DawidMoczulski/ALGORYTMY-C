...
void DodajNowyElWart(lista *l, int y){
    ...
    while ((*l)->nast != NULL && ((*l)->nast)->klucz <= y) {
        l = &((*l)->nast);
    }
    DodajPoczatek(l, y);
}
...
void polacz(lista *l, lista *q, lista *a) {
    while (*l != NULL && *q != NULL) {
        if ((*l)->klucz <= (*q)->klucz) {
            DodajNowyEl(a, (*l)->klucz);
            UsunPierwszy(l);
        } else {
            DodajNowyEl(a, (*q)->klucz);
            UsunPierwszy(q);
        }
    }
...
void polaczWart(lista *l, lista *q, lista *a){
    while ((*l)->klucz != INT_MAX && (*q)->klucz != INT_MAX) {
        if ((*l)->klucz <= (*q)->klucz) {
            DodajNowyEl(a, (*l)->klucz);
            UsunPierwszy(l);
        } else {
    ...
    do{
        ...
        switch(wart){
            ...
            do{
                ...
                printf("8. Polacz ze soba dwie posortowane listy. \n");
                printf("0. Wstecz.\n");
                printf("Podaj numer: ");
                scanf("%d",&wybor);
